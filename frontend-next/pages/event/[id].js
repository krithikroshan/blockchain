...
export default function Event({event, tickets}) {
    ...
    var ticket_min = Math.min.apply( null, tickets.map((v) => v.price))
    var ticket_max = Math.max.apply( null, tickets.map((v) => v.price))

    return (
        <>
        <Layout>
            <div className={styles.blurimage}>
                <Image alt="blurred image background" src={event.image} className={styles.blurredimage}layout="fill"/>
            </div>
            <div className={styles.box} style={{marginTop:30}}>
                <Row>
                    ...
                    <Col md={8} sm={24} xs={24} className={styles.boxdetails}>
                        <span className={styles.boxdate}>{dayjs(event.start_date).format('ddd, D MMM')}</span>
                        <h4 className={styles.boxname}>{event.name}</h4>
                        <p className={styles.boxpricing}>{(ticket_min === ticket_max) ? "$"+ticket_min : "$"+ticket_min + " - " + "$"+ticket_max}</p>
                    </Col>
                </Row>
                <Row>
                    <div className={styles.boxpanel}>
                        ...
                        <Col offset={10} span={8}>
                            <Button type="primary" className={styles.button} onClick={showModal}>See Tickets</Button>
                        </Col>
                    </div>
                </Row>
                <Row>
                    <div className={styles.boxbody}>
                        ...
                        <Col span={8} style={{padding: "0 20px"}}>
                            ...
                            <p style={{marginBottom: 50}}>
                                {dayjs(event.start_date).format('ddd, DD MMMM, YYYY')}<br/>
...
export async function getServerSideProps({ params }) {
    const event = await fetch(`${API_BASE_URL}/events/${params.id}?expand=location`).then(res => res.json())
